...
const Button = React.forwardRef(({ className, variant, type = 'button', children, disabled, ...props }, ref) => {
    ...
    let variantStyles = '';
    if (variant === 'outline') {
        // Ensuring outline buttons also have a black border for consistency
...
const Select = ({ value, onValueChange, children }) => {
    ...
    return (
        <div className="relative">
            ...
            {isOpen && (
                // Removed shadow from Select dropdown
                <div className="absolute z-10 w-full mt-1 overflow-hidden rounded-md border border-border bg-popover text-popover-foreground">
...
const SelectItem = ({ value, children, onClick, currentValue }) => (
    <div 
        onClick={onClick}
        className={`relative flex w-full cursor-default select-none items-center rounded-sm py-1.5 pl-8 pr-2 text-sm outline-none focus:bg-accent focus:text-accent-foreground data-[disabled]:pointer-events-none data-[disabled]:opacity-50 ${currentValue === value ? 'font-semibold bg-accent' : 'hover:bg-accent'}`}
    >
        {children}
    </div>
);
...
export default function SignIn() {
  ...
  const [label, setLabel] = useState('experience_seeker'); // Initial state updated
  ...
  return (
    ...
    <div className="min-h-screen">
      <div className="min-h-screen bg-background flex items-center justify-center p-4">
        {/* Main form container: border-foreground (black) applied, shadow removed */}
        <div className="w-full max-w-sm p-6 bg-white rounded-xl border border-foreground">
          ...
          <div className="flex flex-col items-center mb-4">
            ...
            />
            <h1 className="text-foreground text-4xl mt-3">FOMO</h1>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <Select value={label} onValueChange={setLabel}>
                <SelectTrigger className="bg-background border-border text-foreground">
                  <SelectValue>
                    {label === 'experience_seeker' ? 'Experience Seeker' : label === 'event_organiser' ? 'Event Organiser' : 'Moderator'}
                  </SelectValue>
                </SelectTrigger>
                <SelectContent>
            ...
            <div className="text-left">
              <Link href="#" className="text-sm text-primary hover:underline font-medium transition-colors">
                Forgot password?
              </Link>
            </div>
            ...
            <Button
              type="submit"
              disabled={isLoading}
              // Button is styled to have the required black border and no shadow
              className="w-full bg-primary text-primary-foreground hover:bg-primary/80 transition-colors border border-foreground"
            >
              {isLoading ? (
                <>
            ...
            <Button
              type="button"
              variant="outline"
              // Ensure Google button also has a black border
              className="w-full border-foreground bg-white text-foreground hover:bg-muted/50 transition-colors"
            >
